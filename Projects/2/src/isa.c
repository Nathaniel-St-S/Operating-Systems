...
void sub(const dword instruction)
{
  ...
  if(imm_flag)
  {
   sdword imm = sign_extend(instruction & OPERAND_MASK, 12);
   THE_CPU.registers[dr] = THE_CPU.registers[sr1] - imm;
   set_sub_flags(THE_CPU.registers[sr1], imm, THE_CPU.registers[dr]); 
  }
  else
  {
    // Not in immediate mode so we must get our value from a second source register
...
void mul(const dword instruction)
{
  ...
  if(imm_flag)
  {
    sdword imm = sign_extend(instruction & OPERAND_MASK, 12);
    THE_CPU.registers[dr] = THE_CPU.registers[sr1] * imm;
    set_mul_flags(THE_CPU.registers[sr1], imm, THE_CPU.registers[dr]);
  }
  else
  {
    // Not in immediate mode so we must get our value from a second source register
...
void divide(const dword instruction)
{
  ...
  if(imm_flag)
  {
    ...
    THE_CPU.registers[dr] = THE_CPU.registers[sr1] / imm;
    set_div_flags(THE_CPU.registers[sr1], imm, THE_CPU.registers[dr]);
  }
  else
  {
    // Not in immediate mode so we must get our value from a second source register
...
void get_char()
{
  THE_CPU.registers[BX] = (dword)getchar();
}
...
{
  //one char per dword
  dword* c = RAM + THE_CPU.registers[BX];
  while (*c)
  {
      putc((char)*c, stdout);
      ++c;
...
static inline void interrupt(const dword instruction)
{
  ...
  THE_CPU.registers[PC] = THE_CPU.registers[AX] + 1;
}
