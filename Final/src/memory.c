...
{
  WRS = size;
	RAM = (uint32_t*)malloc(sizeof(uint32_t) * size);
	for(int i = 0; i < size; i++)
	{
		RAM[i] = NO_VAL;
	}
...
void init_SSD(const int size) {
	SSD = (uint32_t*)malloc(sizeof(uint32_t) * size);
	for(int i = 0; i < size; i++)
	{
		SSD[i] = NO_VAL;
	}
...
void init_HDD(const int size) {
	HDD = (uint32_t*)malloc(sizeof(uint32_t) * size);
	for(int i = 0; i < size; i++)
	{
		HDD[i] = NO_VAL;
	}
...
int cache_search(Cache* cache, const uint32_t addr, int* hit_counter, int* miss_counter)
{
...
uint32_t read_mem(const uint32_t addr)
{
	int index;
  // L1 Cache Search
  ...
  // L2 Cache Search
	...
	if(index != EMPTY_ADDR)
	{
		uint32_t val = L2.items[index].val;
...
void write_mem(const uint32_t addr, const uint32_t val)
{
	RAM[addr] = val;
...
uint32_t mallocate(int pid, int size)
{
  ...
  uint32_t old_start_addr = best_fit->start_addr;
  uint32_t old_end_addr = best_fit->end_addr;

  uint32_t new_end_addr = (old_start_addr + size) - 1;
