...
\begin{frame}[fragile]{Instruction Set \& Syscalls}
    \begin{itemize}
        \item MIPS-like ISA with R/I/J formats: arithmetic (add/sub/mult/div), logic (and/or/xor/nor), shifts, compare (\texttt{slti/sltiu}), branches (\texttt{beq/bne}), jumps (\texttt{j/jal/jr/jalr}).
        \item Memory ops: byte/half/word loads with sign/zero extend (\texttt{lb/lbu/lh/lhu/lw}) and stores (\texttt{sb/sh/sw}) routed through cache-backed \texttt{read\_*/write\_*}.
        \item Trap instructions: \texttt{syscall} handles I/O/exit; \texttt{break} halts; \texttt{eret} reserved (stubbed).
    \end{itemize}
\end{frame}
